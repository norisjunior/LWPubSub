...
if [[ $# -eq 0 ]] ; then
    ...
    echo 'example: Very high pool frequency: make_lwpubsub_remote-poll.sh VERYHIGH ORCHESTRA'
    echo ''
    exit 0
fi
...
make TARGET=zoul BOARD=remote-revb PORT=/dev/ttyUSB1 MAKE_KEYSIZE=256 MAKE_CRYPTOMODE=CBC MAKE_WITH_ENERGY=1 MAKE_POLLFREQUENCY="$1" MAKE_WITHSCHEDORCHESTRA=$is_orchestra lwsec-pubsub-system.upload
